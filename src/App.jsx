...
const tab_guides = {
  ppt: {
    title: 'PPT 생성(표준산출물)',
    desc: '표준 파워포인트(.pptx) 산출물 일괄편집, 목차 분석 기반 하위 목차별 분할 생성, 엑셀 데이터 매핑 생성, 스마트 애니메이션, PPT ➜ HWPX 양식 융합, HWP ➔ HWPX 변환 및 MD(마크다운) ➔ MS Word(.docx) 일괄 변환을 통합 제공합니다.',
    steps: [
      '상단 서브 탭에서 원하는 산출물 변환/생성 도구 선택 (PPT 일괄편집, PPT 목차별 분할, 엑셀 매핑, HWPX 변환, MD ➔ Word 변환 등)',
      '대상 파일(.pptx, .xlsx, .hwp, .md 등) 업로드 또는 직접 입력/드래그 앤 드롭',
      '원터치 일괄 자동 변환/분할/생성 수행 및 원하는 폴더 선택/ZIP 저장'
    ],
    tips: 'PPT 목차별 분할 기능은 대용량 PPTX의 목차(대주제/장/간지)를 자동 감지하여 원본 서식과 마스터 슬라이드 손실 없이 개별 PPTX로 깔끔하게 분리합니다.'
  },
  g2b: {
    title: '나라장터 정보조회',
  ...
  'ppt-verify': {
    title: '표준산출물 검증',
    desc: '작성된 표준 산출물 문서(PPTX, HWPX, PDF, MD, Word, Excel) 내의 오탈자, 맞춤법, 금지어, 영한 혼용 및 동일 단어 중복 등의 품질 이슈를 종합 검증합니다.',
    steps: [
      '검증할 문서 파일(PPTX, HWPX, PDF, MD, Word, Excel)을 업로드 영역에 드롭합니다.',
      '검증 항목(오탈자 사전, AI 맞춤법, 금지어/특정단어, 동일단어 중복 등)을 설정합니다.',
      '[문서 검증 시작] 버튼을 클릭해 실시간 정밀 분석을 수행합니다.',
      '검출된 오류 내역과 페이지별 위치를 확인하고 개선안을 검토합니다.'
    ],
    tips: 'PDF, Word(.docx), Markdown(.md), Excel(.xlsx) 등 다양한 포맷의 문서를 일괄 분석할 수 있습니다.'
  },
  main: {
        ...
        <header className="content-header">
          ...
          <div className="header-right">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '2px', marginRight: '8px' }}>
              ...
              <span style={{ fontSize: '12px', color: 'var(--accent-blue)', fontWeight: 700, fontFamily: 'monospace' }}>2026.09.14 v2.13.2</span>
            </div>
